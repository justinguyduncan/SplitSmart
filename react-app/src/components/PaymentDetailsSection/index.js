...
function PaymentDetailsSection({ paymentId }) {
  ...
  const createdDate = `${month[date.getMonth()]} ${date.getDate()}, ${date.getFullYear()} `
  const selectedFriendship = useSelector((state)=>state.friend?.selectedFriendship)
  ...
  return (
    <>
      ...
      <section className="subheader">
        ...
        <p>Added by {selectedFriendship?.user?.short_name} on {createdDate}</p>
