...
function FriendPage() {
    ...
    if (isFriendshipLoaded && isUserExpensesLoaded && isSettledExpensesLoaded && isUnsettledExpensesLoaded && isSentPaymentsLoaded && isReceivedPaymentsLoaded) {
        ...
        unsettledItems = [...userUnsettledExpenses, ...friendUnsettledExpenses].sort((e1, e2) => {
        ...
        settledItems = [...userSettledExpenses, ...friendSettledExpenses, ...userSentPayments, ...userReceivedPayments].sort((e1, e2) => {
            return new Date(e2.created_at).getTime() - new Date(e1.created_at).getTime()
        });
    }
